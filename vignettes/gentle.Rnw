...
cone_inf <- function(dx, dy, ...){  # and allows other args
...
\begin{sloppypar}
Instead of \code{cone\_inf} we could have used a pre-defined IF with suitable parameters. Both \code{gates\_inf} and \code{gnomon\_inf} are cones if \code{a=1} (see \code{?influence}). For instance, \code{gnomon\_inf} implements a general form $S - b R^a$, where $S$ is the mark value, and $R$ is point-to-plant distance. (Names like \code{gnomon.inf} in older versions of \pkg{siplab} have been changed by substituting \code{\_} for \code{.} to prevent overthinking by knowledgeable users: nothing to do with object-oriented programming!) Therefore, \code{ assimilation(threeTrees, influence=gnomon\_inf, infpar=c(a=1, b=1, smark=1))} produces exactly the same result. The argument \code{infpar} specifies parameters for the IF, and \code{smark} identifies the mark to use for size if there are more than one.
\end{sloppypar}
