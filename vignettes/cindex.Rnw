...
Install \pkg{siplab} with \code{install.packages("siplab")} or through clicking on the appropriate GUI menus, if not already installed. This also installs the required \pkg{spatstat} package.
...
\end{equation}
where $S$ is some measure of tree size. Note: in older versions of \pkg{siplab} the kernel function mames ended in \code{.ker} instead of \code{\_ker}.
